...
{ 
    ...
    {
	Verbosity(2) StringCbPrintf(szBuffer, cbDest, "\tAtt %s: no known values\n", AttName[Att]);
	SendOutput (szBuffer);
	...
	Verbosity(2)
	{
	    StringCbPrintf(szBuffer, cbDest, "\tAfter merging single-class subsets:");
		 SendOutput (szBuffer);
	...
	ForEach(V1, 1, Blocks-1)
	{
		...
		Verbosity(4)
		{
		    StringCbPrintf(szBuffer, cbDest, "\tcombine %d %d info %.3f gain %.3f val %.3f",
	...
	Verbosity(2)
	{
	    StringCbPrintf(szBuffer, cbDest, "\t\tform subset ");
		 SendOutput (szBuffer);
	    PrintSubset(Att, Subset[Att][BestV1]);
	    StringCbPrintf(szBuffer, cbDest, ": %d subsets, inf %.3f, gain %.3f, val %.3f\n",
		   Blocks, BestInfo, BestGain, BestVal);
		 SendOutput (szBuffer);
	    Verbosity(3)
	    {
		StringCbPrintf(szBuffer, cbDest, "\t\tcombine %d, %d", BestV1, BestV2);
		SendOutput (szBuffer);
	...
	Verbosity(2) StringCbPrintf(szBuffer, cbDest, "\tFinal subsets:");
	SendOutput (szBuffer);
	Verbosity(3) PrintDistribution(Att, Blocks, false);
	Verbosity(2)
	    StringCbPrintf(szBuffer, cbDest, "\tinf %.3f gain %.3f val %.3f\n", 
...
void PrintSubset(Attribute Att, Set Ss)
{
	...
	if ( In(V1, Ss) )
	{
	    ...
	    StringCbPrintf(szBuffer, cbDest, "%s", AttValName[Att][V1]);
		 SendOutput (szBuffer);
	}
    }
}
